  ...
  uint32_t op_flags)
{
  ...
  while (left > 0 && lp != o->onode.extent_map.end()) {
    ...
    unsigned b_len = std::min(std::min(left,
				       lp->second.length - l_off),
			      bptr->blob.length - b_off);
  ...
  while (b2r_it != blobs2read.end()) {
	...
	[&](uint64_t offset, uint64_t length) {
	  ...
	  _verify_csum(o, &bptr->blob, 0, compressed_bl) < 0) {
	return -EIO;
      }
    ...
    WriteContext *wctx)
{
  ...
  while (ep != o->onode.extent_map.end()) {
	...
	(!b->blob.has_csum_data() ||
	 (b_off % b->blob.get_csum_chunk_size() == 0 &&
	  b_len % b->blob.get_csum_chunk_size() == 0))) {
